class LegislationProposalsController < ApplicationController
  ...
  def index
    @proposals = Proposal.is_legislation_proposal.page(params[:page]).per(5)
  end
  ...
  def show
    ...
    if @proposal.retired_at.blank?
      ...
      redirect_to legislation_proposals_path, notice: 'La propuesta ha sido eliminada.'
    end
  end
  ...
  #     @featured_proposals = Proposal.not_archived.is_legislation_proposal.sort_by_confidence_score.limit(3)
